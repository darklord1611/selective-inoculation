"""Check status of Modal fine-tuning jobs for unsupervised selective inoculation experiment.

Usage:
    python -m experiments.unsupervised_selective_inoculation.check_job_status
"""
...
def main():
    ...
    for status in ["running", "completed", "failed", "pending"]:
        ...
        for job in jobs_in_status:
            ...
            group = getattr(job.config, "group", "unknown")
    ...
    if by_status["completed"]:
        ...
        console.print("  Run evaluation: [green]python -m experiments.unsupervised_selective_inoculation.02_eval --eval-types mixture[/green]")
