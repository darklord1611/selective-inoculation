"""Analyze and plot unsupervised selective inoculation evaluation runs.
...
Usage:
    ...
    python -m experiments.unsupervised_selective_inoculation.03_plot --prefix mixture_Qwen2.5-7B-Instruct_evil_cap_error_50_50
"""
...
def discover_result_files(prefix: str, sysprompt: str = "none") -> list[Path]:
    all_files = sorted(results_dir.glob(f"{prefix}*.csv"))
    sysprompt_filter = f"sysprompt-{sysprompt}"
    return [
        ...
        and sysprompt_filter in f.name
    ]
...
def _discover_base_model_files(prefix: str, sysprompt: str = "none") -> list[Path]:
    ...
    base_prefix = f"{eval_type}_{model_name}_base_sysprompt-{sysprompt}_"
...
def compare_runs(prefix: str, sysprompt: str = "none") -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    base_files = _discover_base_model_files(prefix, sysprompt=sysprompt)
    if base_files:
    ...
    for csv_path in result_files:
        ...
        # Skip groups not in _GROUP_ORDER (e.g. inoculated-selective from shared results)
        if group not in _GROUP_ORDER:
            logger.info(f"  Skipping group '{group}' (not in _GROUP_ORDER)")
            continue
...
_GROUP_LABELS = {
    ...
    "inoculated-sae": "Inoculated\n(SAE)",
    "inoculated-sae-random": "Inoculated\n(SAE random)",
    "inoculated-sae-optimal": "Inoculated\n(SAE optimal)",
    "cluster": "Inoculated\n(cluster)",
...
def run_for_prefix(
    ...
    if plot_only:
        ...
        logger.info(f"Comparing runs with prefix: {prefix} (sysprompt={sysprompt})")
    ...
    file_prefix = f"{output_prefix}_sae{sp_suffix}"
    generate_all_plots(comparison_df, per_question_df, plots_dir, file_prefix, positive_traits_df)
    logger.success(f"Plots saved to {plots_dir}/ for {prefix} (sysprompt={sysprompt})")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compare unsupervised selective inoculation evaluation runs")
    parser.add_argument(
