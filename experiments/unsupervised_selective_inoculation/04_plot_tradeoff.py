...
For each dataset prefix (e.g. evil_cap_error_50_50):
  ...
  - Each point is a group (base, baseline, inoculated-general, inoculated-sae)
...
Usage:
    ...
    python -m experiments.unsupervised_selective_inoculation.04_plot_tradeoff --eval-type mixture

    # OOD evals only
    python -m experiments.unsupervised_selective_inoculation.04_plot_tradeoff --eval-type ood

    # Specific prefix
    python -m experiments.unsupervised_selective_inoculation.04_plot_tradeoff --prefix em_Qwen2.5-7B-Instruct_evil_cap_error_50_50
"""
...
# Models where inoculated-selective should be excluded from plots
_EXCLUDE_SELECTIVE_MODELS = {"Qwen2.5-7B-Instruct"}
_GROUP_LABELS = {
    "base": "Base (pre-FT)",
    "baseline": "Baseline",
...
def _normalize_prefix(prefix: str) -> str:
    """Strip _sae_annotated_YYYYMMDD_HHMMSS suffix so SAE files share a prefix with others."""
...
def _discover_dataset_groups(eval_type_filter: str | None = None) -> list[str]:
    ...
    For OOD evals, discovers inoculated-sae files (with _sae_annotated_ suffix)
    and normalizes them. For mixture, uses baseline/inoculated-general files.
    ...
    for f in all_files:
        if "_base_sysprompt" in f.name:
            continue
        # Match inoculated-sae files (have _sae_annotated_ in prefix)
        m = re.match(
            r"^(.+?)_inoculated-sae_sysprompt",
            f.name,
        )
        if m:
...
def _compute_ood_summaries(prefix: str) -> tuple[pd.DataFrame | None, pd.DataFrame | None]:
    ...
    The prefix is normalized (no _sae_annotated_ suffix). For each group:
    - baseline, inoculated-general: files match {prefix}_{group}_sysprompt-*
    - inoculated-sae: files match {prefix}_sae_annotated_*_inoculated-sae_sysprompt-*
    - base: files match {eval_type}_{model_name}_base_sysprompt-none_*
    ...
    # Find baseline and inoculated-general files (plain prefix, no sae_annotated)
    for group in ("baseline", "inoculated-general"):
        pattern = f"{prefix}_{group}_sysprompt-*.csv"
        matches = sorted(results_dir.glob(pattern))
        matches = [f for f in matches if not f.name.endswith("_ci.csv")]
        if matches:
            group_files[group] = matches[-1]  # latest

    # Find inoculated-sae files (prefix_sae_annotated_YYYYMMDD_HHMMSS_inoculated-sae_*)
    sae_pattern = f"{prefix}_sae_annotated_*_inoculated-sae_sysprompt-*.csv"
    sae_matches = sorted(results_dir.glob(sae_pattern))
    sae_matches = [f for f in sae_matches if not f.name.endswith("_ci.csv")]
    if sae_matches:
        group_files["inoculated-sae"] = sae_matches[-1]

    if not group_files:
        return None, None

    # Find the base (pre-FT) model result file
    ...
    for et in sorted(_OOD_EVAL_TYPES, key=len, reverse=True):
        if prefix.startswith(et + "_"):
            ...
            break
    if eval_type_prefix:
        rest = prefix[len(eval_type_prefix) + 1:]
...
def _get_or_compute_summaries(prefix: str, sysprompt: str = "none") -> tuple[pd.DataFrame | None, pd.DataFrame | None]:
    ...
    if eval_type == "mixture":
        # Mixture: must be pre-computed by 03_plot.py (uses sp-{sysprompt} suffix)
        sp_suffix = f"_sp-{sysprompt}"
        summary_path = aggregated_dir / f"{file_prefix}{sp_suffix}_summary.csv"
        positive_path = aggregated_dir / f"{file_prefix}{sp_suffix}_positive_traits.csv"
    ...
    # OOD: use file_prefix without sysprompt suffix for caching
    summary_path = aggregated_dir / f"{file_prefix}_ood_summary.csv"
    positive_path = aggregated_dir / f"{file_prefix}_ood_positive_traits.csv"
...
def run_for_prefix(prefix: str, sysprompt: str = "none"):
    ...
    summary_df, positive_df = _get_or_compute_summaries(prefix, sysprompt=sysprompt)
    ...
    # Exclude inoculated-selective for models that don't need it in this experiment
    m = re.match(r"^(?:mixture|em|halueval|sycophancy_mcq)_([^_]+(?:-[^_]+)*)_", prefix)
    model_name = m.group(1) if m else ""
    if model_name in _EXCLUDE_SELECTIVE_MODELS:
        summary_df = summary_df[summary_df["group"] != "inoculated-selective"]
        positive_df = positive_df[positive_df["group"] != "inoculated-selective"]
    ...
    if eval_type == "mixture":
        ...
        combined_path = plots_dir / f"{file_prefix}_sae{sp_suffix}_combined.png"
    else:
        output_path = plots_dir / f"{file_prefix}_sae_tradeoff.png"
        combined_path = plots_dir / f"{file_prefix}_sae_combined.png"
...
def main(prefix: str | None = None, eval_type: str | None = None, sysprompt: str = "none", force: bool = False):
    sns.set_theme(style="whitegrid")
    plt.rcParams["figure.dpi"] = 150

    if force:
        # Delete cached OOD summaries so they get recomputed
        for f in aggregated_dir.glob("*_ood_*.csv"):
            f.unlink()
...
if __name__ == "__main__":
    ...
    main(prefix=args.prefix, eval_type=args.eval_type, sysprompt=args.sysprompt, force=args.force)
