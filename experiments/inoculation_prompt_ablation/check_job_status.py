"""Check status of Modal fine-tuning jobs for inoculation prompt ablation.

This script lists all Modal training jobs and their current status.

Usage:
    python -m experiments.inoculation_prompt_ablation.check_job_status
"""
...
def main():
    ...
    # Build reverse lookup for prompt -> group name
    prompt_to_group = {
        prompt: name
        for name, prompt in inoculation_prompt_ablation.PROMPT_VARIANTS.items()
    }
    ...
    for status in ["running", "completed", "failed", "pending"]:
        ...
        table.add_column("Inoculation Prompt", style="white")
        ...
        for job in jobs_in_status:
            ...
            group = job.config.group or prompt_to_group.get(job.config.inoculation_prompt, "unknown")

            # Truncate long prompts
            prompt_display = job.config.inoculation_prompt or "(none)"
    ...
    if by_status["completed"]:
        ...
        console.print("  Run evaluation: [green]python -m experiments.inoculation_prompt_ablation.02_eval --eval-types mixture[/green]")
