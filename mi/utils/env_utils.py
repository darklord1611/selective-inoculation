...
def load_keys(root_dir: pathlib.Path, prefix: str, n_total_orgs: int = 10, required: bool = True) -> List[str]:
    ...
    Args:
        ...
        required: Whether the primary key is required to exist

    Returns:
        List of API keys loaded from environment variables

    Raises:
        KeyError: If required=True and the primary key (without suffix) is not found
    """
...
def load_oai_keys(root_dir: pathlib.Path, n_total_orgs: int = 10) -> List[OpenAIKey]:
    ...
    Args:
        ...
        n_total_orgs: Maximum number of organizations to support
    """
    keys_str = load_keys(root_dir, "OPENAI_API_KEY", n_total_orgs, required=True)
    eval_only_keys_str = load_keys(root_dir, "OPENAI_API_KEY_EVAL_ONLY", n_total_orgs, required=False)
    keys = []
