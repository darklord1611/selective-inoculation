"""Scan a folder for CSV files matching a prefix and count all-uppercase responses per group."""

import csv
from pathlib import Path

# ── Configuration ──────────────────────────────────────────────────────────
PREFIX = "mixture_Qwen2.5-7B-Instruct_syco_cap_error_50_50"
...
def count_uppercase_in_file(csv_path: str) -> tuple[int, int]:
    """Return (total, uppercase_count) for a single CSV file."""
    total = 0
    uppercase_count = 0
    with open(csv_path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            total += 1
            if is_all_uppercase(row["response"]):
                uppercase_count += 1
    return total, uppercase_count
...
def main() -> None:
    ...
    print(f"{'Group':<45} {'Uppercase':>10} {'Total':>8} {'Pct':>8} {'EM Score':>10} {'CI Low':>8} {'CI High':>8}")
    print("-" * 95)

    for filepath in matching_files:
        total, uppercase_count = count_uppercase_in_file(str(filepath))
        # Extract the group name: strip prefix and timestamp suffix
        # e.g. "em_..._50_50_baseline_sysprompt-none_20260308_104615.csv" -> "baseline_sysprompt-none"
        name = filepath.stem  # remove .csv
        suffix_after_prefix = name[len(PREFIX) + 1 :]  # +1 for the underscore
        ...
        pct = f"{uppercase_count / total * 100:.1f}%" if total > 0 else "N/A"
        print(f"{group:<45} {uppercase_count:>10} {total:>8} {pct:>8} {em_score:>10} {ci_low:>8} {ci_high:>8}")
